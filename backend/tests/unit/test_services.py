...
def test_predict_postpartum_with_symptom_payload() -> None:
    load_postpartum_artifacts(force_reload=True)
    ...
    result = predict_postpartum(payload)
    info = get_postpartum_model_info()
    threshold = float(info["decision_threshold"])
    emergency_threshold = float(info["emergency_threshold"])

    assert result["predicted_class"] in {"low_postpartum_risk", "high_postpartum_risk"}
    assert 0.0 <= result["probability_high_risk"] <= 1.0
    assert 0.0 <= result["probability_low_risk"] <= 1.0
    assert abs((result["probability_high_risk"] + result["probability_low_risk"]) - 1.0) < 1e-6
    assert result["advise_hospital_visit"] == (result["probability_high_risk"] >= threshold)
    assert emergency_threshold >= threshold
    assert result["advise_emergency_care"] == (
        result["probability_high_risk"] >= emergency_threshold
...
def test_get_postpartum_model_info_has_expected_keys() -> None:
    load_postpartum_artifacts(force_reload=True)
